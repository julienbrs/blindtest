...
function AudioPathSettings() {
  ...
  return (
    <div className="space-y-4">
      ...
      {status === 'success' && (
        <p className="text-green-400">✓ Chemin valide</p>
      )}
...
<div className="space-y-2">
  <label className="text-sm text-purple-300">Temps pour répondre</label>
...
export async function GET(request: NextRequest) {
  ...
  if (yearMin || yearMax < 9999) {
    songs = songs.filter(
      (s) => s.year && s.year >= yearMin && s.year <= yearMax
    )
  }
